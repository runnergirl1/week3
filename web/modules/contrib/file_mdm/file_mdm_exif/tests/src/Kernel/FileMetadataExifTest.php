...
class FileMetadataExifTest extends FileMetadataManagerTestBase {
  ...
  public function testJpegExifSaveToFile() {
    ...
    $this->assertEquals('N', $file_metadata->getMetadata('exif', 'GPSLatitudeRef')['text']);
    ...
    $this->assertEquals('shot by foo!', $data['Artist']);
    $this->assertFalse(isset($data['Make']));
    $this->assertEquals('12/4', $data['BrightnessValue']);
    $this->assertFalse(isset($data['ISOSpeedRatings']));
    $this->assertEquals('qux', $data['RelatedFileFormat']);
    $this->assertFalse(isset($data['InterOperabilityIndex']));
    $this->assertEquals('N', $data['GPSLatitudeRef']);
    $this->assertEquals(['46/1', '37/1', '59448/10000'], $data['GPSLatitude']);
    $this->assertEquals('E', $data['GPSLongitudeRef']);
  ...
  public function testTiffExifSaveToFile() {
    ...
    $this->assertEquals(4, $file_metadata->getMetadata('exif', 'PhotometricInterpretation')['value']);
